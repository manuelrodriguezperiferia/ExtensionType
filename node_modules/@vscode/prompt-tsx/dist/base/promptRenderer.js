...
class PromptRenderer {
    ...
    constructor(_endpoint, _ctor, _props, _tokenizer) {
        ...
        this._references = [];
        this._tokenizer = _tokenizer ?? new tokenizer_1.Cl100KBaseTokenizer();
    }
    getAllMeta() {
    ...
    async render(progress, token) {
        ...
        const { result: prioritizedChunks } = this._prioritize(resultChunks, (a, b) => MaterializedChatMessageTextChunk.cmp(a, b), (chunk) => {
        ...
        return { messages: this._validate(messageResult), hasIgnoredFiles: this._ignoredFiles.length > 0, tokenCount, references: (0, arrays_1.coalesce)(references) };
    }
    _validate(chatMessages) {
        const lastMessage = chatMessages[chatMessages.length - 1];
        if (lastMessage && lastMessage.role !== openai_1.ChatRole.User) {
...
function computeTokensConsumedByLiterals(tokenizer, element, instance, pieces) {
