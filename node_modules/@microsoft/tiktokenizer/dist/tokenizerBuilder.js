...
const tikTokenizer_1 = require("./tikTokenizer");
const MODEL_PREFIX_TO_ENCODING = new Map([
    // chat
    ["gpt-4-", "cl100k_base"],
    ["gpt-3.5-turbo-", "cl100k_base"] // e.g, gpt-3.5-turbo-0301, -0401, etc.
]);
exports.MODEL_TO_ENCODING = new Map([
    // chat
    ["gpt-4", "cl100k_base"],
 ...
 */
const REGEX_PATTERN_1 = "'s|'t|'re|'ve|'m|'ll|'d| ?\\p{L}+| ?\\p{N}+| ?[^\\s\\p{L}\\p{N}]+|\\s+(?!\\S)|\\s+";
/*
 * regex pattern used after gpt-3.5-turbo
 */
const REGEX_PATTERN_2 = "(?:'s|'S|'t|'T|'re|'RE|'Re|'eR|'ve|'VE|'vE|'Ve|'m|'M|'ll|'lL|'Ll|'LL|'d|'D)|[^\\r\\n\\p{L}\\p{N}]?\\p{L}+|\\p{N}{1,3}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+";
function getEncoderFromModelName(modelName) {
    let encoder = "";
    if (!exports.MODEL_TO_ENCODING.has(modelName)) {
        ...
        encoder = exports.MODEL_TO_ENCODING.get(modelName);
    }
    return encoder;
}
